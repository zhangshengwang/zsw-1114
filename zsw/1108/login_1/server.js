...
mongoose.connect('mongodb://zsw:123@127.0.0.1:27017/login?authSource=admin',{userMongoClient:true});
...
//hash 存储：sessionid:xxxxx login  true
...
app.post("/login",function(req,res,next){
  UserModel.count({name:req.body.user,passwd:req.body.passwd},function(err,num){
    if(num === 1){
      //将session相关信息缓存到redis中
      RedisClient.hmset("sessionid:" + req.sessionID,"name",req.body.user,"passwd",
      req.body.passwd,"login",true,function(err,result){
        if(err){
          console.log(error);
          next(err);
        }
...
app.get("/logout",function(req,res,next){
  //退出时，将会话信息中的登录状态写为false
  RedisClient.hmset("sessionid:" + req.sessionID,"name",req.body.user,"passwd",
req.body.passwd,"login",false,function(err,result){
